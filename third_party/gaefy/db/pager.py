...
"""
    ...
    It is an implementation of the design posted in the App Engine group:
...
class PagerQuery(object):
    ...
    def __init__(self, model_class, keys_only=False):
        ...
        # Properties used to build a bookmark: inequalities and orderings.
        self._bookmark_properties = []
        # First result id or key_name.
    ...
    def _fetch_from_bookmark(self, limit, bookmark):
        ...
        """
        q_values = self._get_query_values(bookmark)
        if  not q_values:
            return []
        ...
        for prop in self._orderings:
            q_filters[prop] = q_values[prop]

        # Fetch results from derived queries starting from bookmark.
        results = []
        q_orderings = []
        ...
        for prop in self._orderings:
            ...
            q_filters.pop(prop)
            if self._order_directions[prop] == '':
                operator = '>'
            else:
                operator = '<'

            q_inequality_filters = {operator: q_values[prop]}
